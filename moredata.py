...
# Making a dataframe to fill in values for the first games of the 2011 season
gamesdata2017 = gamesdata.loc[gamesdata.SEASON >= 2010]

# Removing entries before 2011 season
...
recorddata["SEASON_ID"] = recorddata["SEASON_ID"].apply(lambda x: x - 20000 if x > 20000 else x )
recorddata["SEASON_ID"] = recorddata["SEASON_ID"].apply(lambda x: x - 10000 if x < 20000 and x > 3000 else x)


# Removing entries before 2018 season from recorddata
recorddata = recorddata.loc[recorddata.SEASON_ID >= 2011]
recorddata.sort_values(by=["SEASON_ID"])
recorddata = recorddata.reset_index()

# Adding mean points scored for the home team in the past 20 games
...
for ind in range(len(gamesdata)):
    ...
    if match_stats.empty or prevgamescount < 10:
        # If there are less than 10, then go back to the previous season and take the average
        lastseas = gamesdata2017.loc[
            (gamesdata2017.GAME_DATE_EST < date) & (gamesdata2017.HOME_TEAM_ID == hteam)
        ]
        lastseas10 = lastseas.iloc[:10]
        avgppglast10val = lastseas10.PTS_home.mean(axis=0)
        avgpalast10val = lastseas10.PTS_away.mean(axis=0)
    else:
        # If there are more than 10, take only the most recent 10 
    ...
    if match_stats_away.empty or prevgamescounta < 10:
        lastseasa = gamesdata2017.loc[
            (gamesdata2017.GAME_DATE_EST < date)
            & (gamesdata2017.VISITOR_TEAM_ID == hteam)
        ]
        lastseasa10 = lastseasa.iloc[:10]
        avgppglast10vala = lastseasa10.PTS_away.mean(axis=0)
        avgpalast10vala = lastseasa10.PTS_home.mean(axis=0)
    else:
...
# Taking the average of the last 10 away games and 10 home games
